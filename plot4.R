...
# specify the global graphics parameters
par(mfrow = c(2,2))
...
with(subpower_cons, {
    ...
    plot(subpower_cons$Time, as.numeric(as.character(subpower_cons$Voltage)), type = "l", xlab = "datetime", ylab = "Voltage")
    ...
    plot(subpower_cons$Time, as.numeric(as.character(subpower_cons$Global_reactive_power)), type = "l", xlab = "datetime", ylab = "Global_reactive_power")
})
